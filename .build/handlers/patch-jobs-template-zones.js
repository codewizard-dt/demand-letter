...
var patch_jobs_template_zones_exports = {};
__export(patch_jobs_template_zones_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(patch_jobs_template_zones_exports);
...
var handler = async (event) => {
  ...
  const updated = await Promise.all(
    zones.map(
      (z) => import_db.prisma.zone.update({
        where: { id: z.id },
        data: {
          type: z.type,
