...
**Testing:** `comprehend-client.ts` and `comprehend-medical-client.ts` each export a module-level SDK client singleton. Unit tests use `mockClient(ComprehendMedicalClient)` from `aws-sdk-client-mock` to intercept `DetectPHICommand` calls without network access. See uses::[[aws-sdk-client-mock.md]].
