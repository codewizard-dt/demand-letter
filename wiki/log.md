...
## [2026-06-30] research | Multi-variable zone refactor
Completed codebase research on refactoring Zone model to support multiple variables per zone and the same variable in multiple zones. Filed as `raw/research/multi-variable-zone-refactor/index.md`. Key finding: injection machinery already handles multi-variable templateText; the broken parts are the generation loop (String.replace replacing only one field), the classifier prompt (only producing one suggestedFieldName), and the injector deduplication logic (clearSet suppressing shared-field zones). No DB migration needed.

## [2026-06-30] task | TASK-112 Refactor Zone model: elevate templateText to primary source of truth for multi-variable zones
Created task TASK-112: fix the Zone 1:1 field-name limitation by elevating templateText to the primary rendering source — 7 phases covering the generation loop helper, classifier prompt update, removal of applyDistinctDuplicateSuffixes and clearSet dedup, lazy migration in the patch handler, frontend ZoneConfigCard, and test/eval updates.

## [2026-06-30] task-done | TASK-112 Refactor Zone model — multi-variable zones (all 7 steps complete)
Implemented all 7 steps of TASK-112: (1) Added `renderZoneContent` helper in routes/rest.ts, fixing 4 broken `String.replace(literal)` sites so all `{field}` tokens in templateText are substituted; (2) Updated LLM classifier prompt and rewrote `normalizeClassification` to validate and strip non-canonical fields from multi-variable templateText; (3) Removed `applyDistinctDuplicateSuffixes` and its helpers from zone-classifier.ts; (4) Removed clearSet/lastSeenAt deduplication from docx-injector.ts::injectDelimiters; (5) Added lazy migration in patch-jobs-template-zones.ts and routes/rest.ts — null templateText auto-promoted to {suggestedFieldName} on save; (6) Updated ZoneConfigCard: templateText textarea is now primary, shows "Variables detected" chips, and has an "Insert variable" quick-add; (7) Updated zone-classifier.test.ts (removed suffix-dedup assertions, added multi-variable test), docx-system-fields.test.ts (renamed clearSet test). Typecheck clean throughout. No DB migration needed.
