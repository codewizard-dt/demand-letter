...
- [ ] Create the file with a single exported async function:
  ```ts
...
- [ ] Throw `Error('No extracted fields found for job ${jobId}')` if `rows.length === 0`.

### 2. Re-export from `packages/api/src/lib/index.ts`  <!-- agent: general-purpose -->

- [ ] Open `packages/api/src/lib/index.ts`.
- [ ] Add: `export { buildDataObject, type GenerationData } from './generation-data-builder';`

### 3. Typecheck  <!-- agent: general-purpose -->

- [ ] Run `pnpm --filter @demand-letter/api typecheck` (or `tsc --noEmit`) from the repo root.
