...
- [ ] Identify where `invokeModelStream` is called and chunks are collected.
...
- [ ] Replace the existing `invokeModelStream` call with:
  ```ts
  const userId = 'system';
  ...
  ```
- [ ] Log grounding report to console (non-blocking warning already emitted inside the function).
...
- [ ] Return:
  ...
  ```
- [ ] Update `jobs.status = 'done'` and `jobs.output = narrativeText` (the narrative text without SSE formatting) BEFORE returning the response. The output S3 key is set in TASK-044 (docxtemplater render).
