...
- **Both PHI *and* PII must be scrubbed from logs**: HIPAA covers PHI; SOC 2 covers all PII. Two SDK calls per document.
...
- **Encryption ≠ redaction.** Neither HIPAA nor SOC 2 requires redacting the *primary store* — that's KMS-backed RDS + S3 SSE-KMS. Redaction is for **logs** and **developer-facing views**. Keep the full encrypted block text in Postgres for the attorney citation flow; mask dev views by default using tagged offsets. Over-redacting the provenance store breaks citations.
...
- **Complements** DEC-0003#D2 (Claude-on-Bedrock keeps PHI in-account during *inference*) — but Bedrock does NOT cover PHI sitting in CloudWatch logs or readable Postgres rows. This decision closes that gap. Not a supersession.
