...
- **Model-size figures in `index.md` §3** (spaCy `en_core_web_lg` ≈ 560 MB; transformers de-id model ≈ 400+ MB) are approximate, widely-documented package sizes used to show that a base-spaCy + transformers install exceeds the 250 MB Lambda zip limit (S7/S8). The _conclusion_ (container image required; cold-start risk) is grounded in S5/S7/S8; the exact megabyte figures are order-of-magnitude estimates, not pinned to a single authoritative byte count.
